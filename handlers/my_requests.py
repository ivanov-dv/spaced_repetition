...
async def show_my_requests(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    user_requests = await request_repo.get_requests_for_user(
    ...
    if not user_requests:
        ...
        await callback.message.edit_text(
            assist.list_requests_to_text(user_requests),
            reply_markup=MyRequestKb.my_requests()
        )


@router.callback_query(F.data == 'mr_delete')
async def ask_nums_for_delete_request(
    ...
    msg = await callback.message.edit_text(
        f'{assist.list_requests_to_text(user_requests)}\n\n'
...
async def delete_request(message: types.Message, state: FSMContext):
    ...
    if not request_nums:
        await msg.edit_text(
            f'<code>❗️ {"Некорректные значения":^25} ❗️\n'
    ...
    if flag:
        await msg.edit_text(
            '❗️ Ошибка. Не все задачи были удалены ❗️',
            reply_markup=KB.main()
        )
    await msg.edit_text(
        f'✅ Задачи № [{", ".join(map(str, request_nums))}] удалены.',
        reply_markup=KB.main()
    )


@router.callback_query(F.data == 'mr_delete_all')
async def delete_all_requests(callback: types.CallbackQuery):
    await callback.message.edit_text(
        '⁉️ Вы уверены, что хотите удалить все напоминания?',
        reply_markup=MyRequestKb.delete_all_confirm()
    )


@router.callback_query(F.data == 'mr_delete_all_confirm')
async def delete_all_requests_confirm(
        callback: types.CallbackQuery,
        state: FSMContext
):
    data = await state.get_data()
